...
from typing import Any, Callable, Optional, Sized, Union, List, Tuple, Dict
...
class SpectusGRPOTrainer(transformers.Seq2SeqTrainer):
    """
    Adam - chnged the father class from Trainer to Seq2SeqTrainer
    ...
    def __init__(
        ...
        **kwargs, # Adam - for compute_metrics
        ...
        if isinstance(model, str):
            ...
            # Check if we are loading an encoder-decoder model or a causal LM
            is_encoder_decoder = model_init_kwargs.get("is_encoder_decoder", False)
            if is_encoder_decoder:
                model = AutoModelForSeq2SeqLM.from_pretrained(model, **model_init_kwargs)
            else:
        ...
        if self.beta == 0.0:
            ...
            if is_encoder_decoder:
                self.ref_model = AutoModelForSeq2SeqLM.from_pretrained(model_id, **model_init_kwargs)
            else:
        ...
        self.max_completion_length = args.max_completion_length  # = |o_i| in the GRPO
        self.num_generations = args.num_generations  # = G in the GRPO paper - group size
        self.temperature = 1.0 if args.temperature is None else args.temperature
        self.top_p = args.top_p
        self.top_k = args.top_k
        self.min_p = args.min_p
        self.do_sample = args.do_sample
        self.penalty_alpha = args.penalty_alpha
        ...
        if self.use_vllm:
            ...
            # Configure generation settings, which differ slightly between causal LM and encoder-decoder models
            ...
            # If we're working with an encoder-decoder model, add decoder_start_token_id
            if self.is_encoder_decoder:
                # Get decoder_start_token_id from model config if available, otherwise use bos_token_id
                decoder_start_token_id = getattr(
                    model.config, "decoder_start_token_id", processing_class.bos_token_id
                )
                generation_config_args["decoder_start_token_id"] = decoder_start_token_id
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In SpectusGRPOTrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    def _get_per_token_logps(self,
        ...
        # For encoder-decoder, we're only interested in the decoder part, which already has the right size
        input_ids = decoder_input_ids[:, 1:]  # Skip the first token and match logits shape
    ...
    def _generate_and_score_completions(
        ...
        # thorougly changed to support Spectus model

        device = self.accelerator.device
        # potentially delete this vvv
        ...
        # repeat the encoder inputs for each generation (decoder inputs are already repeated from generation)
        input_ids = inputs["input_ids"].repeat_interleave(self.num_generations, dim=0)
        position_ids = inputs["position_ids"].repeat_interleave(self.num_generations, dim=0)
        attention_mask = inputs["attention_mask"].repeat_interleave(self.num_generations, dim=0)
        labels_ids = inputs["labels"].repeat_interleave(self.num_generations, dim=0)
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1:
                old_per_token_logps = self._get_per_token_logps(
                    self.model, input_ids, position_ids, attention_mask, completion_ids, completion_mask, logits_to_keep
                )
            else:
            ...
            if self.beta == 0.0:
                ...
                ref_per_token_logps = self._get_per_token_logps(
                    self.ref_model, input_ids, position_ids, attention_mask, completion_ids, completion_mask, logits_to_keep
                )
            else:
        ...
        # Decode the generated completions and labels
        completions_text = self.processing_class.batch_decode(completion_ids, skip_special_tokens=True)
        # replace -100 with padding token
        labels_positive = labels_ids.masked_fill(labels_ids == -100, self.processing_class.pad_token_id)
        labels_text = self.processing_class.batch_decode(labels_positive, skip_special_tokens=True)

        rewards_per_func = torch.zeros(len(input_ids), len(self.reward_funcs), device=device) # Adam inputs -> input_ids
        for i, (reward_func, reward_processing_class) in enumerate(
            zip(self.reward_funcs, self.reward_processing_classes)
        ):
            reward_func_name = reward_func.__name__
            with profiling_context(self, reward_func_name): # this block is weird. ADAM
                    # Repeat all input columns (but "prompt" and "completion") to match the number of generations
                    output_reward_func = reward_func(prompts=None, completions=completions_text, labels=labels_text) # Adam no need for prompts in e-d
        ...
        if torch.isnan(rewards_per_func).all(dim=1).any():
            ...
            row_reward_kwargs = {"label": labels_ids[nan_row_idx], "completion": completions_text[nan_row_idx]}
            warnings.warn(
        ...
        if self.log_completions and self.state.global_step % self.args.logging_steps == 0:
            ...
            labels_to_log = gather_object(labels_text)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The SpectusGRPOTrainer does not support returning outputs")
        ...
        per_token_logps = self._get_per_token_logps(model, input_ids, position_ids, attention_mask, completion_ids, completion_mask, logits_to_keep)

        # in e-d completion works differently - we need to cut the first (BOS) token that does not
        # have a counterart in logits to match the shape of the logits
        shifted_completion_mask = inputs["completion_mask"][:, 1:]
        ...
        per_token_logps_shape = per_token_logps.shape  # smaz pak
        loss = (per_token_loss * shifted_completion_mask).sum() / shifted_completion_mask.sum()

        # Log the metrics
        mode = "eval" if self.control.should_evaluate else "train"

        if self.beta != 0.0:
            mean_kl = (per_token_kl * shifted_completion_mask).sum() / shifted_completion_mask.sum()
            self._metrics[mode]["kl"].append(self.accelerator.gather_for_metrics(mean_kl).mean().item())

        is_clipped = (coef_1 < (1 - self.epsilon_low)) | (coef_1 > (1 + self.epsilon_high))
        clip_ratio = (is_clipped * shifted_completion_mask).sum() / shifted_completion_mask.sum()
    ...
    def prediction_step(
        ...
        **gen_kwargs,
        ...
        """
        Copied from transformers.trainer_seq2seq.Seq2SeqTrainer.prediction_step to handle input for
        generation properly (differs from training -- we dont want to use _prepare_inputs).
        ...
        if has_labels:
            inputs["labels"] = inputs["labels"].long()  # Adam: Convert labels to long type
        # inputs = self._prepare_inputs(inputs)
...
def morgan_tanimoto_reward_function(prompts, completions, labels=None, **kwargs):
    ...
    Args:
        prompts (list): List of prompts (not used in this function).
    ...
    Returns:
        list: List of Tanimoto similarity scores between 0 and 1, or None if computation fails.
    """
    ...
    # Use radius=2 and fpSize=2048 for Morgan fingerprints (recommended values)
    similarities = compute_fp_simils(
        clean_completions,
        labels,
        fp_type="morgan",
